...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    val sorted: List[Char] = chars.sortWith((x, y) => x < y)

    def groupAll(current: (Char, Int), tail: List[Char]): List[(Char, Int)] = {
      if (tail.isEmpty)
        List(current)
      else if (tail.head == current._1)
        groupAll((current._1, current._2 + 1), tail.tail)
      else {
        List(current) ::: groupAll((tail.head, 1), tail.tail)
      }
    }
    ...
    if (sorted.isEmpty)
      ...
      groupAll((sorted.head, 1), sorted.tail)
  }

  /*
  def times(chars: List[Char]): List[(Char, Int)] = {
    val countChars: Map[Char, Int] = chars.sortWith( _ < _).groupBy(identity).map(x => (x._1, x._2.size))
    countChars.toList
  }
  */
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    if (trees.length < 2)
      ...
      val firstTwo: CodeTree = makeCodeTree(trees.head, trees.tail.head)

      val merged: List[CodeTree] = (firstTwo :: trees.tail.tail).sortWith((x, y) => weight(x) < weight(y))
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case left :: right :: tail => new Fork(left, right, chars(left) ::: chars(right), weight(left) + weight(right)) :: combine(tail)
    case _ => trees
  }
  */
  ...
  /**
    ...
    */
  @tailrec
  def until(singlet: List[CodeTree] => Boolean, comb: List[CodeTree] => List[CodeTree])(treeList: List[CodeTree]): List[CodeTree] = {
    if (singlet(treeList)) // only one tree left
      treeList
    else
      until(singlet, comb)(comb(treeList))
  }
  ...
  def createCodeTree(chars: List[Char]): CodeTree = {
    val charPairs: List[(Char, Int)] = times(chars)
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    ...
    def help(subTree: CodeTree, nextBit: List[Bit]): List[Char] = {
      subTree match {
  ...
  */
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    def help(subTree: CodeTree, nextChars: List[Char]): List[Bit] = {
  ...
  */
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    def help(next: List[Char]): List[Bit] = {

      if (next isEmpty)
        List[Bit]()
      else
        codeBits(table)(next.head) ::: help(next.tail)
    }
