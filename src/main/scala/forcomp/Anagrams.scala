...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toCharArray.groupBy((c: Char) => c.toLower).map(x => (x._1, x._2.length)).toList.sorted
  /*
  {
    val r = w groupBy identity map (x => (x._1.toLower, x._2.length)
    r.toList.sorted
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    def helper (occurrences: Occurrences, ch: Char): Occurrences = {
    ...
    val subSet = for( occ <- occurrences ) yield combinations(helper(occurrences, occ._1))

    occurrences :: (subSet.flatten.distinct)
  }
